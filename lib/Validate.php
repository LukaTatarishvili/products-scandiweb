...
class Validate
{
    ...
    {
        return !(filter_var($data['price'], FILTER_VALIDATE_FLOAT) && (strlen($data['price']) > 0) && floatval($data['price'] >= 0));
    }

    public function validateType($data)
    {
        return !(preg_match('/[1-3]/', $data['product_type']) && (strlen($data['product_type']) > 0));
    }
    ...
    {
       return is_numeric($data['height']) && is_numeric($data['width']) && is_numeric($data['length']) && floatval($data['height'] >= 0) && floatval($data['width'] >= 0) && floatval($data['length'] >= 0);
    }
